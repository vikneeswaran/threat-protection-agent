...
#     Write-Host "â•‘  Kuamini Security Client - Windows Installer               â•‘"
...
#     try {
#         $decoded = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Token))
#         Write-Log "âœ“ Token is valid base64" "SUCCESS"
#     }
#     catch {
...
#         Write-Log "âœ“ Token is valid JSON" "SUCCESS"
#     }
#     catch {
...
#         }
#     }
#     Write-Log "âœ“ Token contains all required fields (accountId, timestamp)" "SUCCESS"
...
#         }
#         else {
#             Write-Log "âœ“ Token timestamp is valid (age: $([Math]::Round($ageDays, 1)) days)" "SUCCESS"
#         }
#     }
#     catch {
...
#        $msiSizeRounded = [Math]::Round($msiSize, 2)
#        Write-Log "[OK] MSI downloaded successfully ($msiSizeRounded MB)" "SUCCESS"
#         return $msiPath
#     }
#     catch {
...
#                 Write-Log "âœ“ Token written to: $tokenFile" "SUCCESS"
#                 $tokenWritten = $true
#             }
#         }
...
#             Write-Log "âœ“ Executable found: $exePath" "SUCCESS"
#             $found = $true
#             break
#         }
#     }
...
#     Write-Log "âœ“ Configuration file verified: $script:CONFIG_FILE" "SUCCESS"
#     return $true
# }
...
#                 Write-Log "âœ“ Agent started successfully" "SUCCESS"
#                 $agentStarted = $true
...
#     Write-Host ""
#     Write-Host "â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—"
#     Write-Host "â•‘  Installation Completed Successfully!                      â•‘"
#     Write-Host "â•‘  Agent ID: $agentId"
#     Write-Host "â•‘  Status: Agent is running and registering...               â•‘"
#     Write-Host "============================================================"
#     Write-Host ""
...
$script:API_BASE_URL = "https://kuaminisystems.com/api/agent"
$script:MSI_DOWNLOAD_URL = "https://github.com/vikneeswaran/threat-protection-agent/raw/refs/heads/main/public/tray/KuaminiSecurityClient-1.0.26-windows.zip"
$script:MSI_TEMP_DIR = Join-Path $env:TEMP "kuamini-install-$(Get-Random)"
$script:CONFIG_DIR = Join-Path $env:LOCALAPPDATA "KuaminiSecurityClient"
$script:CONFIG_FILE = Join-Path $script:CONFIG_DIR "config.json"
...
function ConvertFrom-TokenJSON {
    ...
    try {
        $normalized = $Token.Trim().Replace("-", "+").Replace("_", "/")

        switch ($normalized.Length % 4) {
            0 { }
            2 { $normalized += "==" }
            3 { $normalized += "=" }
            1 { throw "Invalid base64/base64url token length." }
        }
        ...
        Write-Log "Could not decode token as base64/base64url JSON: $($_.Exception.Message)" "WARN"
        return $null
    }
}
...
function Validate-RegistrationToken {
    ...
    # The server generates installation tokens as exactly
    # 128 characters using Base64URL encoding.
    if ($Token.Length -ne 128) {
        Write-ErrorLog "Invalid registration token length. Expected exactly 128 characters, got $($Token.Length)."
        return $false
    }

    # Validate that the token contains only Base64URL characters.
    # Base64URL uses A-Z, a-z, 0-9, '-' and '_'.
    if ($Token -notmatch '^[A-Za-z0-9_-]{128}$') {
        Write-ErrorLog "Invalid registration token format. Token contains characters outside the Base64URL character set."
        return $false
    }

    Write-Log "[OK] Token format is valid (128-character Base64URL token)" "SUCCESS"

    # IMPORTANT:
    # The installation token is opaque. It is not JSON and does not
    # contain accountId or timestamp information.
    #
    # Actual token validation is performed by the server during
    # POST /api/securityagent/agent/register.
    #
    # The server checks:
    #   1. Token exists in installation_tokens
    #   2. Token has not expired
    #   3. Associated account exists
    #   4. Account is active
    #   5. Account has an available license
...
function Get-InstallerMSI {
    param(
        ...
        [Parameter(Mandatory = $false)]
        [string]$InstallerUrl = $script:MSI_DOWNLOAD_URL
    )

    Write-Log "Downloading Windows installer..." "INFO"

    try {
        New-Item -ItemType Directory -Path $script:MSI_TEMP_DIR -Force -ErrorAction Stop | Out-Null

        $archivePath = Join-Path $script:MSI_TEMP_DIR "KuaminiSecurityClient.zip"
        $extractPath = Join-Path $script:MSI_TEMP_DIR "extracted"

        Write-Log "Downloading installer archive..." "INFO"
        ...
        if (-not (Test-Path -LiteralPath $archivePath)) {
            throw "Installer archive was not created."
        }

        $archiveSize = (Get-Item -LiteralPath $archivePath -ErrorAction Stop).Length / 1MB
        $archiveSizeRounded = [Math]::Round($archiveSize, 2)
        Write-Log "[OK] Installer archive downloaded successfully ($archiveSizeRounded MB)" "SUCCESS"

        New-Item -ItemType Directory -Path $extractPath -Force -ErrorAction Stop | Out-Null

        Write-Log "Extracting installer archive..." "INFO"
        ...
        if (-not $msi) {
            throw "No MSI file was found inside the downloaded installer archive."
        }

        Write-Log "[OK] MSI found: $($msi.FullName)" "SUCCESS"
        return $msi.FullName
    }
    catch {
        Write-ErrorLog "Failed to download/extract installer: $($_.Exception.Message)"
        return $null
    }
}
# ============================================================================
    ...
    try {
        ...
        foreach ($installPath in $installPaths) {
            if (Test-Path $installPath) {
                $tokenFile = Join-Path $installPath "registration.token"
                Set-Content -Path $tokenFile -Value $Token -Encoding UTF8 -Force
                Write-Log "[OK] Token written to: $tokenFile" "SUCCESS"
                $tokenWritten = $true
            }
        }
...
function Test-Installation {
    ...
    foreach ($installPath in $installPaths) {
        $exePath = Join-Path $installPath "KuaminiSecurityClient.exe"
        if (Test-Path $exePath) {
            Write-Log "[OK] Executable found: $exePath" "SUCCESS"
            $found = $true
            break
        }
    }
    ...
    Write-Log "[OK] Configuration file verified: $script:CONFIG_FILE" "SUCCESS"
    return $true
}
...
function Main {
    ...
    # Step 2.5: Validate token if supplied
    Write-Host ""
    ...
    # Step 7: Write registration token
    Write-Host ""
